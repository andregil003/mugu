...
// degradado verde institucional, cards redondeadas y botones animados.
  ...
  return (
    <div className="relative mx-auto w-full max-w-md px-4 py-10">
      ...
      <div className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
        <div className="absolute -top-24 left-1/2 h-72 w-72 -translate-x-1/2 rounded-full bg-gradient-to-br from-green-500/20 via-emerald-500/20 to-teal-500/20 blur-3xl" />
        <div className="absolute bottom-0 right-0 h-56 w-56 rounded-full bg-gradient-to-br from-emerald-500/10 to-teal-500/10 blur-3xl" />
      </div>
      ...
      <div className="mt-6 rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
        <p className="mb-4 flex items-center justify-center gap-2 text-center text-sm font-semibold text-muted-foreground">
          <FontAwesomeIcon icon={faLanguage} className="h-4 w-4 text-emerald-600" />
          {t('seleccionaIdioma')}
        </p>
        <div className="space-y-3">
          {idiomas.map((i) => (
            <button
              ...
              className="group flex w-full items-center justify-between rounded-2xl border border-gray-200 bg-white px-5 py-4 text-left shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-emerald-300 hover:shadow-lg hover:shadow-green-500/10 active:scale-[0.98]"
            >
              <span className="text-lg font-bold">{i.nombre}</span>
              <span className="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-100 text-emerald-700 transition-all duration-200 group-hover:bg-emerald-600 group-hover:text-white group-hover:translate-x-0.5">
